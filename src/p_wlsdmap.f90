...
!                                                               !                                                        
! Output                                                        !                                                        
! 1. Eigenvalues and eigenvectors (diffusion coordinates)       !                                                        
!    of Fokker-Planck operator                                  !                                                        
!                                                               !                                                        
!---------------------------------------------------------------!                                                       
! Develop log                                                   !                                                        
!                                                               !                                                        
...
!       The matrix are stored in Compressed Sparse Row(CSR)     !                                                        
!         structure.                                            !                                                        
!                                                               !                                                        
!       WZ 5/12/2009                                            !                                                        
!       Parallel version                                        !                                                        
!       PARPACK package is used to find the eigenvalues of the  !                                                        
!        matrix.                                                !                                                        
!                                                               !                                                        
!       CC 12/10/2007                                           !                                                        
...
subroutine Weighted_LSDMap
  ...
  use parallel, only :  size, rank, ierr, comm, trace_exit
  ...
  real(kind=8), allocatable :: LocalScale(:)
  real(kind=8), allocatable :: lScale(:)
  real(kind=8), allocatable :: EV(:,:)
  real(kind=8), allocatable :: EVAll(:,:)
  ...
  real(kind=8), allocatable :: dloc(:)
  real(kind=8), allocatable :: sumrow(:)
  real(kind=8), allocatable :: sumloc(:)
  real(kind=8), allocatable :: vv(:)
  real(kind=8), allocatable :: v_all(:)
  real(kind=8), allocatable :: ww(:)
  ! tmp_rmsd: temporarily store the rmsd data from the input file
  real,allocatable :: rmsd(:)
  ! Sparse storage
  integer, allocatable :: jdxloc(:)
  integer, allocatable :: iloc(:)
  ! Input and output file name
  character(180) :: NN_input_rmsd
  character(180) :: NN_input_eps
  character(180) :: NN_input_loc
  ...
  allocate (workd(3*maxnloc))
  allocate (d(maxncv,2))
  allocate  (resid(maxnloc))
  allocate (ax(maxnloc))
  allocate (mv_buf(maxnloc))
  ...
  ! Write to output file from processor 0  
  if (rank == 0) then
  ...
  if(status_dmap==1) then 
     ...
     call trace_exit("MPI_Bcast for Weight failed",ierr)
  else
     Weight=1.
  endif
  ...
  call trace_exit("MPI_Allgather for NlocPoint failed",ierr)
  ...
  if(status_eps==1) then 
     ...
     call trace_exit("MPI_Bcast for LocalScale failed",ierr)
  else
     LocalScale=eps0
  endif
  call MPI_barrier(comm,ierr)
  call trace_exit("MPI_Barrier in p_wlsdmap failed",ierr)
  ...
  if(count>sumi) then
     write(*,*)'Warning: core #',rank,' does not have enough memory to store the sparse matrix'
     write(*,*)'core #',rank,' has used',count,'/',sumi, ' sparse storage'
     write(*,*)'Please increase the value nspace in the program'
     stop
  endif
  write(*,*)'core #',rank,' has used',count,'/',sumi, '% sparse storage'
  ...
  call trace_exit("MPI_Allgtaherv for sumloc failed",ierr)
  ...
  !call MPI_barrier(comm, ierr)
  call MPI_ALLGATHERV(sumloc,NlocPoint,MPI_DOUBLE_PRECISION,sumrow,NallPoint,idxlocPoint(1:size)-1,MPI_DOUBLE_PRECISION,comm,ierr)
  call trace_exit("MPI_Allgatherv for sumloc failed",ierr)
...
10 continue
  ...
  if (ido .eq. -1 .or. ido .eq. 1) then
     ...
     call trace_exit("MPI_Allgatherv for vv failed",ierr)
  ...
  if ( info .lt. 0 ) then
     ...
     if ( ierr .ne. 0) then
        ...
        do j=1, nconv
           ...
           call trace_exit("MPI_Allgatherv for v failed",ierr)
     ...
     if (rank .eq. 0)then
        if ( info .eq. 1) then
           ...
           write(*,*) ' ' 
           write(*,*) ' No shifts could be applied during implicit Arnoldi update, try increasing NCV.'
           write(*,*) ' '
        end if
        write(*,*) ' '
        write(*,*) '_SDRV1 '
        ...
        write(*,*) ' The number of processors is ', size
...
9000 continue
  ...
  do jdx=1,nconv
     ...
     call trace_exit("MPI_Gatherv for EV failed",ierr)
  enddo
