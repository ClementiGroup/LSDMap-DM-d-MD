...
!                                                               !
! Output                                                        !
! 1. If write_rmsd, the pairwise RMSD matrix and                !
!    nearest neighboring map in binary format are written.      !
!                                                               !
!---------------------------------------------------------------! 
! Develop log                                                   !
!                                                               !
...
subroutine p_rmsd_neighbor
  ...
  use iso_c_binding
  use ftn_c
  use parallel, only : rank, ierr, comm, displacements, counts, trace_exit
  ...
  character(200) :: nn_output
  character(200) :: nn_neigh
  ...
  if(rank==0)  write(*,*)'start calculating pairwise rmsd and nearest neighbor graph'
  weight=1.
  ...
  do idx=nstart,nend
     ...
     if(Nneigh/=0) then
        ...
        call Qsort(tmpdist,localidneigh) 
        reci = idx-nstart+1
        do jdx=1,Nneigh
     ...
     endif ! Closing if(Nneigh/=0)
  enddo ! Closing do idx=nstart,nend

  if (write_rmsd) close(51)
  if(Nneigh/=0 .and. write_neighbor) close(52)
  
  
  ! Array 'tmp_rmsd' o be used in p_wlsdmap.f90, gather all arrays 'dist' from all PEs into one big array
  ! 'tmp_rmsd', available on all PEs
  call MPI_ALLGATHERV(dist,Npoints*nloc,MPI_REAL8,tmp_rmsd,Npoints*counts,displacements,MPI_REAL8,comm,ierr)
  call trace_exit("MPI_Allgatherv of dist failed",ierr) 
