        ...
        while(current_n_coord < ncoords):
          ...
          for index in range(nnodes): 
            if current_n_coord < ncoords:
               ...
               p=subprocess.Popen("sed '"+str(start)+','+str(end)+"!d' "+str(args.grofile)+' > tmp/core'+str(index)+"/tmp.gro; cd tmp/core"+str(index) +"; aprun -n  "+str(nthreads_gromacs)+" -N  "+str(nthreads_gromacs)+" -d 2 grompp_mpi "+additional+" -f ../../grompp.mdp -c tmp.gro -p ../../topol.top -maxwarn 2; aprun "+str(nthreads_gromacs)+" -N  "+str(nthreads_gromacs)+" -d 2 mdrun_mpi 1>/dev/null 2>/dev/null; rm \#*",shell=True)
